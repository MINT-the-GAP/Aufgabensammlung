...
tags: Erklärung, Mengen, Zahlenverständnis
...
{{|>}} Eine weitere *Mengenoperation* ist die *Schnittmenge* $\cap$:

$$
\mathbb{M} \cap \mathbb{K} = \{ 3, 4 \} \quad \text{Schnittmenge von $\mathbb{M}$ und $\mathbb{K}$}
$$

{{|>}} Die *Schnittmenge* wird gelesen als:  
"Alle Zahlen, die sich in $\mathbb{M}$ **und** in $\mathbb{K}$ befinden."

<center>
![Schnittmenge von zwei Mengen. Schwarz: $\mathbb{M}$, rot umrandet: $\mathbb{K}$. Die ausgefüllte Fläche zeigt den Schnittmenge.](Kap1/Durchschnitt.png)
</center>


{{|>}} In diesem Fall sind nur die $3$ und die $4$ in der resultierenden *Schnittmenge*.
...
{{|>}} Es wird deutlich, dass bei der *Differenz* von *Mengen* die Reihenfolge entscheidend ist, während sich die *Schnittmenge* und die *Vereinigung* nicht verändern.
